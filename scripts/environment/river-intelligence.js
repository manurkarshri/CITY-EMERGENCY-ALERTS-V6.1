...
export async function buildRiverIntelligence(riverData = {}) {
  ...
  for (const item of riverData.items || []) {
    ...
    results.push({ id: item.id || `river-${item.dam}-${String(item.lastUpdated || "").slice(0,10)}`, kind: item.kind || "reservoir", dam: item.dam, damLabel: dam.label, river: dam.river, status: item.status, trend: item.trend, storagePercent: item.storagePercent, reservoirLevel: item.reservoirLevel, dischargeCumecs: item.dischargeCumecs, dataFreshness: item.freshness, lastUpdated: item.lastUpdated, source: item.source, sourceUrl: item.sourceUrl, measurementType: item.measurementType || "live_telemetry", storageOnly: Boolean(item.storageOnly), rsi, severity, downstreamLocalities: dam.downstreamLocalities, talukas: dam.talukas, bridges: dam.bridges, travelSensitivity: dam.travelSensitivity, timeline: createTimeline("monitoring", `${dam.label}: ${storage}.`), evidence: [createEvidence({ source: item.source, sourceTrust: item.sourceTrust, type: "official", observedAt: item.lastUpdated })], impact: `${dam.label} has ${storage}. Storage alone is not treated as a flood warning.`, recommendedAction: "Monitor official discharge and downstream warnings." });
  }
  return results;
}
